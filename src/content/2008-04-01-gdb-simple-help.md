...
한 줄씩 진행하는 명령은 두 가지 동작으로 나뉜다. 그것은 현재 수행할 명령 행의 특성에 따라 나뉘는데, 예를 들어 현재 수행할 명령이 함수 호출 명령일 때 해당 함수에 대한 symbol table이 있어서 jumping이 가능하면 그 내부의 진행 상황까지 디버깅할 수 있다. 이 경우 그 함수 내부로 진입(_step into_)하여 디버깅을 진행할 것인지, 아니면 그냥 지나쳐(_step over_) 갈 것인지 선택할 수 있다. 전자의 진입(step into)의 명령은 `step`이고, 축약은 `s`이다. 후자의 진행(step over)의 명령은 `next`이고 축약은 `n`이다. 두 명령 모두 뒤에 숫자를 입력하여 몇 개의 명령이나 진행할 것인지 지정할 수 있다.

...
_짧은 지식으로는 더 잘 설명할 재간이 없으니 [...] 그냥 그렇다고 넘어가자._
...
- 소켓 생성과 동시에 연결(connect)를 할 수 있다.
  ...
  _ 이 때 첫 번째 인자가 연결할 서버의 **hostname 혹은 ip address**이고 두 번째 인자가 **port 번호**이다.
- 연결 후 데이터 통신을 위한 **InputStream**과 **OutputStream**을 가져와 통신할 수 있다.
- 통신이 끝난 후 스트림과 소켓을 **닫아주면** 된다. \* 효율적인 자원관리를 위해 **다 쓴 자원은 꼭 주자.**
